...
- **拥抱工具调用，机械逻辑下沉进 tool**——固定规则（掷骰、寻路、查表、状态机）封装进 `preset/tools/` 由脚本执行，而非让模型用自然语言复现。双重收益：一是把「按规则计算」这类本不该由 LLM 承担的思考成本剥离出去，省 token，同时消除机械逻辑靠生成复现带来的执行不稳定；二是模型每回合都读到 tool 返回的实时客观结果，配合脚本注入，叙事始终有动态的客观参照物，不必凭空编造
