...
describe('mock LLM server wire behaviors', () => {
  ...
  it('draws pool texts reproducibly under a seed and covers every entry', async () => {
    const options = {
      sequence: ['success'] as const,
      repeatLast: true,
      randomSeed: 7,
      chunkSize: 100,
      successTexts: ['alpha response', 'beta response', 'gamma response'],
    }
    const first = await startMockLlmServer(options)
    const second = await startMockLlmServer(options)
    running.push(first, second)

    const read = async (server: MockLlmServer): Promise<string[]> => {
      const bodies: string[] = []
      for (let attempt = 0; attempt < 18; attempt += 1) bodies.push(await (await chat(server)).text())
      return bodies
    }
    const firstBodies = await read(first)

    expect(firstBodies).toEqual(await read(second))
    for (const text of options.successTexts) {
      expect(firstBodies.some(body => body.includes(`"content":"${text}"`))).toBe(true)
    }
  })

  it('keeps the default and single-text behavior when no pool is configured', async () => {
    const defaulted = await start(['success'], { chunkSize: 100 })
    expect(await (await chat(defaulted)).text()).toContain('"content":"mock response recovered"')

    const single = await start(['success'], { successText: 'recovered', chunkSize: 100 })
    expect(await (await chat(single)).text()).toContain('"content":"recovered"')
  })

  it('draws pool texts for every success-shaped behavior', async () => {
    const server = await start(['max_tokens', 'slow_success', 'wrong_content_type'], {
      successTexts: ['only pool text'],
      chunkSize: 100,
      chunkDelayMs: 1,
    })

    for (let attempt = 0; attempt < 3; attempt += 1) {
      const body = await (await chat(server)).text()
      expect(body).toContain('"content":"only pool text"')
    }
    expect(server.requests.every(record => record.outcome === 'completed')).toBe(true)
  })

  it('delays each accepted request by the configured latency before responding', async () => {
    const server = await start(['success'], { latencyMinMs: 50, latencyMaxMs: 50 })
    ...
    expect(server.requests[0]).toMatchObject({ behavior: 'success', outcome: 'completed' })
  })

  it('dwells for the reasoning gap between the reasoning phase and the reply', async () => {
    const server = await start(
      ['reasoning_success'],
      { reasoningText: 'think', chunkDelayMs: 0, reasoningGapMs: 250, latencyMinMs: 0, latencyMaxMs: 0 },
    )
    const startedAt = Date.now()

    const response = await chat(server)
    // Headers resolve early on a streaming response — drain the body so the
    // measurement covers the mid-stream reasoning gap.
    await response.text()
    const elapsedMs = Date.now() - startedAt

    expect(response.status).toBe(200)
    // The gap pause is the only configured wait: reasoning chunks are instant
    // (chunkDelayMs 0), so the whole dwell is the reasoning→content gap.
    expect(elapsedMs).toBeGreaterThanOrEqual(200)
    expect(server.requests[0]).toMatchObject({ behavior: 'reasoning_success', outcome: 'completed' })
  })

  it('sends configured reasoning on success only when the request enables thinking', async () => {
    const server = await start(['success', 'success'], { successText: 'answer', reasoningText: 'think' })

    const thinkingOff = await (await chat(server)).text()
    const thinkingOn = await (await chat(server, {
      body: JSON.stringify({ model: 'mock', messages: [], stream: true, thinking: { type: 'enabled' } }),
    })).text()

    expect(thinkingOff).not.toContain('reasoning_content')
    expect(thinkingOn).toContain('"reasoning_content":"think"')
    expect(thinkingOn).toContain('"content":"answer"')
  })

  it('draws per-request latency from a range', async () => {
    const server = await start(['success', 'success'], { latencyMinMs: 5, latencyMaxMs: 25 })
  ...
  it('records a client that disconnects during the latency wait', async () => {
    const server = await start(['success'], { latencyMinMs: 200, latencyMaxMs: 200 })
    const controller = new AbortController()
    const pending = chat(server, { signal: controller.signal })
    setTimeout(() => { controller.abort() }, 20)
    await expect(pending).rejects.toThrow()
    await new Promise((resolve) => { setTimeout(resolve, 50) })

    expect(server.requests[0]).toMatchObject({ outcome: 'client_closed' })
  })
  ...
  it('rejects invalid method, route, bearer token, and JSON without consuming the script', async () => {
    ...
    // Only the accepted empty-body request reaches the telemetry events; its body is undefined and no headers are echoed.
    expect(events).toEqual([
      {
        type: 'request',
        attempt: 1,
      ...
      },
      {
        ...
        outcome: 'completed',
        chunksSent: 5,
        assistantText: 'mock response recovered',
      },
    ])
  })
})
...
describe('mock LLM server option validation', () => {
  it.each([
    ...
    [{ sequence: ['success'], successTexts: [] }, /successTexts must not be empty/],
    [{ sequence: ['success'], successTexts: ['kept', ''] }, /empty text/],
    ...
    [{ sequence: ['success'], latencyMinMs: -1 }, /latencyMinMs/],
    [{ sequence: ['success'], latencyMinMs: 100, latencyMaxMs: 50 }, /latencyMaxMs/],
