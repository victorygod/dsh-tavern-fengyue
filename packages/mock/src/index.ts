 ...
 *
 * @module dsh-tavern-fengyue-mock
 */
...
export type MockLlmServerEvent =
  | {
    ...
    /** Parsed JSON request body; `undefined` for an empty body. Headers are not echoed here — they stay on the server handle. */
  ...
  | {
    ...
    /** Reasoning text streamed this attempt; present only when configured. */
    readonly reasoning?: string
    /** Full assistant text streamed this attempt. */
...
export interface MockLlmServerOptions {
  ...
  /**
   * Complete texts returned by success-shaped behaviors; one is drawn per
   * request from the seeded random stream shared with behavior selection.
   * Takes precedence over {@link successText}; a single entry reproduces it.
   */
  readonly successTexts?: readonly string[]
  ...
  /** Delay between the last reasoning delta and the first content delta, in milliseconds. */
  readonly reasoningGapMs?: number
  ...
  /**
   * Lower bound of the per-request delay before the first response byte,
   * drawn per request from the seeded random stream shared with behavior and
   * pool selection. Defaults to zero.
   */
  readonly latencyMinMs?: number
  /** Upper bound of the pre-response delay; defaults to {@link latencyMinMs}. */
...
interface ResolvedOptions {
  ...
  readonly partialText: string
  readonly reasoningText: string
  /** Whether the caller explicitly configured reasoning (vs the built-in default). */
...
function resolveOptions(options: MockLlmServerOptions): ResolvedOptions {
  ...
  const reasoningGapMs = boundedInteger(
    'reasoningGapMs',
    options.reasoningGapMs ?? 0,
    0,
    MAX_MOCK_LLM_TIMER_DELAY_MS,
  )
  ...
  const latencyMinMs = boundedInteger('latencyMinMs', options.latencyMinMs ?? 0, 0, MAX_MOCK_LLM_TIMER_DELAY_MS)
  const latencyMaxMs = boundedInteger('latencyMaxMs', options.latencyMaxMs ?? latencyMinMs, 0, MAX_MOCK_LLM_TIMER_DELAY_MS)
  if (latencyMaxMs < latencyMinMs) {
    throw new Error('llm-mock-server: latencyMaxMs must not be below latencyMinMs')
  }
  ...
  const reasoningConfigured = options.reasoningText !== undefined
  ...
  if (options.successText === '') throw new Error('llm-mock-server: successText must not be empty')
  if (options.successTexts !== undefined && options.successTexts.length === 0) {
    throw new Error('llm-mock-server: successTexts must not be empty')
  }
  const successTexts = options.successTexts !== undefined && options.successTexts.length > 0
    ? [...options.successTexts]
    : [options.successText ?? DEFAULT_SUCCESS_TEXT]
  if (successTexts.some(text => text.length === 0)) {
    throw new Error('llm-mock-server: successTexts must not contain an empty text')
  }
...
function finishRecord(
  ...
  outcome: MockLlmRequestOutcome,
  payload?: { reasoning?: string; assistantText?: string },
): void {
...
/**
 * Whether the request enables thinking — the DeepSeek wire marker
 * `thinking.type: 'enabled'`. Requests without the marker read as
 * thinking-off and receive no reasoning phase.
 */
function requestThinkingEnabled(record: MockLlmRequestRecord): boolean {
  const body = record.body as { thinking?: { type?: unknown } } | undefined
  return body?.thinking?.type === 'enabled'
}

/**
 * Stream the reasoning phase shared by `success` and `reasoning_success`:
 * paced `reasoning_content` deltas, then the `reasoningGapMs` dwell before
 * the reply's first content delta. `false` means the client went away.
 */
async function streamReasoning(
  ...
  for (const chunk of splitText(options.reasoningText, options.chunkSize)) {
    ...
    if (!await pause(options.chunkDelayMs, response)) return false
  }
  return pause(options.reasoningGapMs, response)
}
...
async function completeText(
  ...
  finishRecord(options, record, 'completed', {
    ...
    // text in the same attempt); assistantText is the streamed reply.
    ...options.reasoningConfigured ? { reasoning: options.reasoningText } : {},
    assistantText: text,
  })
}
...
async function runBehavior(
  ...
  switch (record.behavior) {
    ...
    case 'success':
      openSse(response)
      // Explicitly configured reasoning rides contentful successes whose
      // REQUEST enables thinking (the DeepSeek wire marker `thinking.type`;
    ...
    case 'max_tokens':
      openSse(response)
      await completeText(options, record, response, 'length', 0, pickSuccessText)
      return
    case 'slow_success':
      openSse(response)
      await completeText(options, record, response, 'stop', options.chunkDelayMs, pickSuccessText)
      return
  }
}
...
export async function startMockLlmServer(options: MockLlmServerOptions): Promise<MockLlmServer> {
  ...
  // A single-entry pool skips the draw so existing seeded behavior sequences reproduce unchanged.
  const pickSuccessText = (): string => {
    if (resolved.successTexts.length === 1) return resolved.successTexts[0] as string
    return resolved.successTexts[Math.floor(random() * resolved.successTexts.length)] as string
  }

  // A degenerate range skips the draw for the same reproducibility reason.
  const pickLatencyMs = (): number => {
    if (resolved.latencyMinMs === resolved.latencyMaxMs) return resolved.latencyMinMs
    return resolved.latencyMinMs + Math.floor(random() * (resolved.latencyMaxMs - resolved.latencyMinMs + 1))
  }
  ...
  const handle = async (request: IncomingMessage, response: ServerResponse): Promise<void> => {
    ...
    // Provider-latency model: the drawn delay precedes every response, HTTP errors included.
    const latencyMs = pickLatencyMs()
    if (latencyMs > 0 && !await pause(latencyMs, response)) return
    await runBehavior(resolved, record, request, response, pickSuccessText)
  }
