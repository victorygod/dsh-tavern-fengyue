...
The repository script writes JSONL to stdout: a `ready` record carries the `/v1` base URL and random seed, followed by request/result records that name both the scripted behavior and the concrete behavior selected; each `request` record carries the parsed request body (system prompt, messages, tools, and sampling parameters), while headers stay on the in-process `server.requests` records only. The package exposes no installable binary.
...
| `success` / `slow_success` / `reasoning_success` | Stream a complete text response, optionally delayed or preceded by paced reasoning and a reasoning gap; `success` reasons only when the request enables thinking, `reasoning_success` always reasons |
...
The CLI exposes `--success-text` (repeatable; each occurrence joins the response pool), `--success-text-file <path>` (one text per line; blank lines are skipped), `--partial-text`, `--reasoning-text`, `--chunk-size`, `--chunk-delay-ms`, `--reasoning-gap-ms` (delay between the last reasoning delta and the first content delta), `--disconnect-delay-ms`, `--latency-ms <min>-<max>` (a single value pins it), `--retry-after-ms`, `--request-id`, `--tool-name`, `--tool-arguments`, and `--show-tools` (pretty requests also print each tool's full wire schema after the name list). Millisecond delays are bounded integers within Node's timer range; `retryAfterMs` must also be positive. Success-shaped behaviors draw one pool entry per request from the seeded random stream shared with `random` behavior selection, so a run reproduces from its seed; a single-entry pool streams that fixed text. Each accepted request waits a latency drawn from the `--latency-ms` range before its first response byte, HTTP errors included. The library accepts the same camel-case options (`successTexts` takes precedence over `successText`; `latencyMaxMs` defaults to `latencyMinMs`). An optional exact `apiKey` validates `Authorization: Bearer <token>`; omission accepts any token. For interactive sessions, the repository script `scripts/mock-llm.sh` bundles a narrative pool, success-heavy failure weights, a 1–3 s latency range, and streamed chunks behind environment overrides.
