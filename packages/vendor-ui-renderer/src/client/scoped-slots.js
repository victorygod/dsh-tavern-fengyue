...
class SlotErrorBoundary extends Component {
    ...
    render() {
        if (this.state.failed)
            return _jsx("div", { "data-slot-error": this.props.slotKey });
...
function standardKit(host, entry, scope, rootBinding, scopeBinding) {
    ...
    if (entry.locale !== undefined) {
        ...
        if (face === undefined) {
            throw new SlotAssemblyError(`entry declares locale namespace '${entry.locale}' but no locale face is installed (locale plugin missing from the composition?)`);
        }
...
function ContextualEntry({ slotKey, Comp, kit, standard, injected, slotInjected, ownerProps, hookContext, hasHookContext, }) {
    ...
    return _jsx(Comp, { ...kit, ...injected, ...slotInjected.props, ...contextual, ...ownerProps });
}
function renderEntry(slotKey, Comp, kit, standard, injected, slotInjected, ownerProps, hookContext, hasHookContext) {
    if (slotInjected.slotHookFactories === undefined) {
        return _jsx(Comp, { ...kit, ...injected, ...slotInjected.props, ...ownerProps });
    }
...
function renderOutletContent(host, slotKey, ownerProps, opts, scopeBinding) {
    ...
    const guarded = (entry, key, owner = ownerProps) => {
        ...
        return spec.scope === 'session'
            ? (_jsx(StrictSessionEntry, { slotKey: slotKey, entry: entry, ownerProps: owner, slotInjected: slotInjected, hookContext: hookContext, hasHookContext: hasHookContext, onEntryError: onEntryError }, key))
            : (_jsx(SlotErrorBoundary, { slotKey: slotKey, onEntryError: onEntryError, children: spec.scope === 'session-maybe'
                    ? (_jsx(SessionMaybeEntry, { entry: entry, ownerProps: owner, slotKey: slotKey, slotInjected: slotInjected, hookContext: hookContext, hasHookContext: hasHookContext }))
                    : (_jsx(RootEntry, { entry: entry, ownerProps: owner, slotKey: slotKey, slotInjected: slotInjected, hookContext: hookContext, hasHookContext: hasHookContext })) }, key));
    };
    ...
    const deadCell = () => _jsx("div", { "data-slot-error": slotKey });
    if (spec.kind === 'single') {
        const entry = host.entriesOfSlot(slotKey)[0];
        if (!entry)
            return entries.length > 0 ? deadCell() : _jsx(_Fragment, { children: opts?.fallback ?? null });
    ...
    if (spec.kind === 'keyed') {
        ...
        if (!entry) {
            ...
            return occupied ? deadCell() : _jsx(_Fragment, { children: opts?.fallback ?? null });
        }
        return guarded(entry, entryKeyOf(entry));
    }
    if (spec.kind === 'chain') {
    ...
    return (_jsx(_Fragment, { children: list.map((item, i) => item.entry !== undefined
...
function RootOutlet({ ownerProps }) {
    ...
    if (!entry) {
        ...
        if (host.entriesOf('root').length > 0)
            return _jsx("div", { "data-slot-error": "root" });
...
//# sourceMappingURL=scoped-slots.js.map